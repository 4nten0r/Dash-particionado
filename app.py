...
if st.session_state.get("authentication_status") == False:
    ...
    try:
        ...
        if menu_selecionado == "Resumo Executivo":
            ...
            st.write("---")
            st.markdown("**🏭 Danos x Faltas por CD de Origem**")
            fig_cd = plot_comparativo_cd_tipo(df_uni)
            if fig_cd:
                st.plotly_chart(fig_cd, use_container_width=True)
            else:
                st.info("Sem CD de Origem identificado para os filtros atuais.")
            ...
            # ==================== BUSCA POR ROTA ====================
            if 'Rota' in df_uni.columns and not df_uni.empty:
                _inval_rota = ['N/A', 'NAN', 'NÃO IDENTIFICADO', 'NAO IDENTIFICADO', '']
                rotas_disp = sorted(
                    r for r in df_uni['Rota'].dropna().astype(str).str.strip().unique()
                    if r.upper() not in _inval_rota
                )
                rota_busca = st.selectbox(
                    "🔎 Buscar rota:", options=rotas_disp, index=None,
                    placeholder="Digite ou selecione o número da rota...",
                    help="Mostra motoristas, danos e faltas da rota. Respeita o período da barra lateral."
                )
                if rota_busca:
                    df_r = df_uni[df_uni['Rota'].astype(str).str.strip() == rota_busca].copy()
                    if df_r.empty:
                        st.info("Sem ocorrências para essa rota no período selecionado.")
                    else:
                        def _moda_val(s, default='—'):
                            m = s[~s.astype(str).isin(['Não Identificada', 'Não Identificado', 'nan', ''])].mode()
                            return m.iloc[0] if not m.empty else default
                        cid_r = _moda_val(df_r['Cidade']) if 'Cidade' in df_r.columns else '—'
                        bai_r = _moda_val(df_r['Bairro']) if 'Bairro' in df_r.columns else '—'
                        fil_r = _moda_val(df_r['Filial']) if 'Filial' in df_r.columns else '—'
                        st.markdown(f"**Rota {rota_busca}** · {cid_r} / {bai_r} · Filial: {fil_r}")

                        d_r = df_r[df_r['Tipo_Ocorrencia'] == 'Dano']
                        f_r = df_r[df_r['Tipo_Ocorrencia'] == 'Falta']
                        k1, k2, k3 = st.columns(3)
                        k1.metric("📦 Danos", f"{int(d_r['Quantidade'].sum())} itens", f"{len(d_r)} ocorrências", delta_color="off")
                        k2.metric("📉 Faltas", f"{int(f_r['Quantidade'].sum())} itens", f"{len(f_r)} ocorrências", delta_color="off")
                        k3.metric("📊 Total", f"{int(df_r['Quantidade'].sum())} itens", f"{len(df_r)} ocorrências", delta_color="off")

                        # Motoristas que atuaram na rota
                        st.markdown("**🚛 Motoristas nesta rota**")
                        _inval_mot = ['NÃO IDENTIFICADO', 'NAO IDENTIFICADO', 'NAN', '', 'N/A']
                        df_rm = df_r[~df_r['Motorista'].astype(str).str.upper().isin(_inval_mot)]
                        if not df_rm.empty:
                            mot = df_rm.pivot_table(index='Motorista', columns='Tipo_Ocorrencia',
                                                    values='Quantidade', aggfunc='sum', fill_value=0).reset_index()
                            for c in ('Dano', 'Falta'):
                                if c not in mot.columns:
                                    mot[c] = 0
                            mot['Total'] = mot['Dano'] + mot['Falta']
                            ocorr = df_rm.groupby('Motorista').size().reset_index(name='Ocorrências')
                            mot = mot.merge(ocorr, on='Motorista', how='left').sort_values('Total', ascending=False)
                            mot = mot.rename(columns={'Dano': '📦 Danos', 'Falta': '📉 Faltas'})
                            st.dataframe(mot[['Motorista', '📦 Danos', '📉 Faltas', 'Total', 'Ocorrências']],
                                         use_container_width=True, hide_index=True)
                        else:
                            st.caption("Sem motorista identificado nas ocorrências desta rota.")

                        # Detalhamento das ocorrências (Pedido · Motorista · Qtd) — sem cliente
                        st.markdown("**📄 Ocorrências da rota**")
                        det = df_r.sort_values('Data_Filtro') if 'Data_Filtro' in df_r.columns else df_r.copy()
                        det = det.copy()
                        if 'Data_Filtro' in det.columns:
                            det['Data'] = pd.to_datetime(det['Data_Filtro'], errors='coerce').dt.strftime('%d/%m/%Y')
                        det = det.rename(columns={'Tipo_Ocorrencia': 'Tipo', 'Quantidade': 'Qtd'})
                        cols_det = [c for c in ['Data', 'Tipo', 'Pedido', 'Motorista', 'Qtd'] if c in det.columns]
                        st.dataframe(det[cols_det], use_container_width=True, hide_index=True)

                        # ---- Exportar em Excel (Data como data real p/ filtrar na gerência) ----
                        exp = df_r.copy()
                        if 'Data_Filtro' in exp.columns:
                            exp['Data'] = pd.to_datetime(exp['Data_Filtro'], errors='coerce')
                        exp = exp.rename(columns={'Tipo_Ocorrencia': 'Tipo', 'Quantidade': 'Qtd'})
                        cols_exp = [c for c in ['Data', 'Rota', 'Filial', 'Cidade', 'Bairro', 'Tipo', 'Pedido', 'Motorista', 'Qtd'] if c in exp.columns]
                        exp = exp[cols_exp]
                        if 'Data' in exp.columns:
                            exp = exp.sort_values('Data')
                        buf = BytesIO()
                        with pd.ExcelWriter(buf, engine='openpyxl') as _w:
                            exp.to_excel(_w, sheet_name='Ocorrencias', index=False)
                            if not df_rm.empty:
                                mexp = df_rm.pivot_table(index='Motorista', columns='Tipo_Ocorrencia',
                                                         values='Quantidade', aggfunc='sum', fill_value=0).reset_index()
                                for c in ('Dano', 'Falta'):
                                    if c not in mexp.columns:
                                        mexp[c] = 0
                                mexp['Total'] = mexp['Dano'] + mexp['Falta']
                                mexp = mexp.rename(columns={'Dano': 'Danos', 'Falta': 'Faltas'}).sort_values('Total', ascending=False)
                                mexp.to_excel(_w, sheet_name='Motoristas', index=False)
                        st.download_button(
                            f"⬇️ Exportar rota {rota_busca} (Excel)", data=buf.getvalue(),
                            file_name=f"rota_{rota_busca}_ocorrencias.xlsx",
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                            key=f"dl_rota_{rota_busca}",
                        )
                st.write("---")
            # ==================== FIM DA BUSCA ====================
