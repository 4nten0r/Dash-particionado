...
def plot_comparativo_cd_tipo(df):
    """Compara Danos x Faltas por CD de Origem (volume de itens). Ordena pelo CD
    de maior volume total; ignora registros sem CD identificado."""
    if df.empty or 'CD_Origem' not in df.columns:
        return None

    base = df[~df['CD_Origem'].astype(str).isin(['Não Identificado', 'N/A', 'nan', ''])].copy()
    if base.empty:
        return None

    df_grp = base.groupby(['CD_Origem', 'Tipo_Ocorrencia'])['Quantidade'].sum().reset_index()

    ordem = (
        df_grp.groupby('CD_Origem')['Quantidade'].sum()
        .sort_values(ascending=False).index.tolist()
    )
    df_grp['CD_Origem'] = pd.Categorical(df_grp['CD_Origem'], categories=ordem, ordered=True)
    df_grp = df_grp.sort_values('CD_Origem')
    ...
    fig = px.bar(
        ...
        title="Danos x Faltas por CD de Origem"
    )
    fig.update_layout(
        xaxis_title="",
