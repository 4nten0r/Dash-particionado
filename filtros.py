...
def aplicar_filtros_barra_lateral(df_uni_base, df_danos_base, df_faltas_base):
    ...
    with st.sidebar:
        ...
        opcoes_cd_origem = sorted([str(x) for x in df_uni_base["CD_Origem"].dropna().unique() if str(x) not in ['Não Identificado', 'nan', 'N/A', '']])
        ...
        cd_origem_sel = st.multiselect("🏭 CD de Origem:", options=opcoes_cd_origem, placeholder="Todos os CDs...")
    ...
    if len(cd_origem_sel) > 0:
        df_uni = df_uni[df_uni["CD_Origem"].isin(cd_origem_sel)]
        df_danos = df_danos[df_danos["CD_Origem"].isin(cd_origem_sel)]
        df_faltas = df_faltas[df_faltas["CD_Origem"].isin(cd_origem_sel)]
