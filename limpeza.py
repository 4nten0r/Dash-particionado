...
try:
    ...
    # Seleciona colunas por NOME (robusto a mudanças de ordem/layout da base da Natura,
    # ex: "Base_Reclamação" tem as colunas em ordem diferente de "Base_R")
    df_base.columns = [str(c).strip() for c in df_base.columns]
    colunas_desejadas = [
        'data', 'descricao_produto', 'qtd_reclamada', 'centro_distribuicao',
        'cliente', 'cod_cliente', 'descricao_ocorrencia', 'pedido', 'id_rota',
        'transportadora', 'filial', 'desvio_logistica', 'empresa', 'categoria',
    ]
    faltando = [c for c in colunas_desejadas if c not in df_base.columns]
    if faltando:
        print(f"⚠️  ATENÇÃO: colunas ausentes na base de danos: {faltando}")
        print("   O layout da base da Natura pode ter mudado — verifique os nomes das colunas.")
    df_base_filtrada = df_base[[c for c in colunas_desejadas if c in df_base.columns]].copy()
...
try:
    ...
    # Cópias completas (histórico todo) para preencher as colunas mensais JAN-DEZ
    # O filtro de período abaixo afeta apenas o ranking dos Top 10, não os dados mensais
    df_danos_full  = df_danos_std.copy()
    df_faltas_full = df_faltas_std.copy() if not df_faltas_std.empty else pd.DataFrame()
    ...
    # Usa df_*_full para que as colunas mensais mostrem o histórico completo,
    # independente do período filtrado para seleção do Top 10
    processa_top_motoristas(top_danos, df_danos_full, "Danos", linhas_danos, colunas_finais, meses_map)
    if not df_faltas_full.empty:
        processa_top_motoristas(top_faltas, df_faltas_full, "Faltas", linhas_faltas, colunas_finais, meses_map)
